...
class SupabaseAppState extends ChangeNotifier {
  ...
  void updateCurrentUser(SupabaseUser user) {
    _currentUser = user;
    notifyListeners();
  }
  ...
  // Method to create sample matches for real donations
  void _createMatchesForRealDonations() {
    if (_donations.isEmpty || _needs.isEmpty) return;
    
    // Clear existing matches to avoid duplicates
    _matches.clear();
    
    // Create matches for some of the real donations
    final realDonations = _donations.take(5).toList(); // Take first 5 donations
    final sampleNeeds = _needs.take(3).toList(); // Use first 3 needs
    
    for (int i = 0; i < realDonations.length && i < 8; i++) {
      final donation = realDonations[i];
      final need = sampleNeeds[i % sampleNeeds.length]; // Cycle through needs
      
      // Create a match with varying scores
      final matchScore = 0.95 - (i * 0.05); // Decreasing match scores
      final statuses = [MatchStatus.pending, MatchStatus.accepted, MatchStatus.pending];
      
      final match = Match(
        id: 'match_real_$i',
        donationId: donation.id, // Use real donation ID
        needId: need.id,
        ...
        acceptedAt: statuses[i % statuses.length] == MatchStatus.accepted 
    ...
    if (kDebugMode) {
      print('Created ${_matches.length} matches for real donations');
      for (final match in _matches) {
        print('Match ${match.id}: donation ${match.donationId} -> need ${match.needId}');
      }
    }
  }
  ...
  Future<void> loadPublicData() async {
    try {
      ...
      // Create matches for real donations after data is loaded
      _createMatchesForRealDonations();
  ...
  void startLoginFlow() {
    if (kDebugMode) {
      print('AuthFlow: Starting login flow.');
    }
    _loginState = ApplicationLoginState.emailAddress;
    notifyListeners();
  }
  ...
  void startRegisterFlow() {
    if (kDebugMode) {
      print('AuthFlow: Starting registration flow.');
    }
  ...
  void cancelRegistration() {
    if (kDebugMode) {
      print('AuthFlow: Cancelling registration, returning to login start.');
    }
    _loginState = ApplicationLoginState.emailAddress;
    notifyListeners();
  }
      ...
      try {
        ...
        if (responseData['status'] is String) {
          ...
          newDonation = Donation(
            ...
            contextDescription: responseData['context_description'] as String?,
  ...
  Future<void> deleteDonation(String donationId) async {
    try {
      ...
      // Also remove any matches for this donation
      _matches.removeWhere((m) => m.donationId == donationId);
      
      notifyListeners();
      
      if (kDebugMode) {
        print('Donation deleted successfully: $donationId');
      }
    } catch (error) {
      if (kDebugMode) {
  ...
  Future<void> fetchAllDonations() async {
    try {
      ...
      // If database has donations, use them; otherwise keep sample data for demo
      if (validDonations.isNotEmpty) {
        _donations = validDonations;
        notifyListeners();
        if (kDebugMode) {
          print('Successfully loaded ${_donations.length} donations from database');
        }
      } else {
        if (kDebugMode) {
          print('Database has no donations, keeping sample data for demo');
        }
      }
    } catch (error) {
      if (kDebugMode) {
  ...
  Future<void> fetchAllNeeds() async {
    try {
      ...
      // If database has needs, use them; otherwise keep sample data for demo
      if (validNeeds.isNotEmpty) {
        _needs = validNeeds;
        notifyListeners();
        if (kDebugMode) {
          print('Successfully loaded ${_needs.length} needs from database');
        }
      } else {
        if (kDebugMode) {
          print('Database has no needs, keeping sample data for demo');
        }
      }
    } catch (error) {
      if (kDebugMode) {
